...
public class MeuAdapterConsoles extends RecyclerView.Adapter<MeuAdapterConsoles.MeuHolder> {
    private ArrayList<DtoConsoles> consoles;
    private OnProdListener onProdListener;

    public MeuAdapterConsoles(ArrayList<DtoConsoles> consoles, OnProdListener onProdListener) {
    ...
    public void onBindViewHolder(@NonNull MeuHolder holder, int position) {
        holder.txtNome.setText(consoles.get(position).getNome());
        holder.txtPreco.setText("R$: " + consoles.get(position).getPreco());
        Picasso.get().load(consoles.get(position).getIdImagem()).into(holder.imgProd);
